...
app.layout = html.Div(children=[html.H1('SpaceY Launch Records Dashboard',
                                ...
                                options=[
                                    ...
                                    {'label': 'CCAFS LC-40', 'value': 'CCAFS LC-40'}, # Double check if it's LC-40 or SLC-40
...
def get_scatter_chart(entered_site, payload_range):
    # 1. Filter dataframe by payload range from the slider
    low, high = payload_range
    mask = (spacex_df['Payload Mass (kg)'] >= low) & (spacex_df['Payload Mass (kg)'] <= high)
    filtered_df = spacex_df[mask]
    
    if entered_site == 'ALL':
        # 2. If ALL sites are selected, plot all filtered data
        fig = px.scatter(filtered_df, x="Payload Mass (kg)", y="class", 
                         color="Booster Version Category",
                         title='Correlation between Payload and Success for all Sites')
        return fig
    else:
        # 3. If a specific site is selected, filter by site as well
